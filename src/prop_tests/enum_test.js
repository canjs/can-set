...
QUnit.test('enum set.intersection', function(assert) {
   ...
   * z ∈ (X ∩ Y) | z.type = 'prep'
	...
	assert.deepEqual(res, {type: ['new','prep'] }, "empty v array intersection");
...
QUnit.test('enum set.difference', function(assert) {
	...
	assert.deepEqual(res, false, "difference from a superset");

	/*
	 * {} = {type: []}
	 */
	res = set.difference({type: [] } , { type: ['new','prep'] }, prop);
...
QUnit.test('enum set.union', function(assert) {
	 ...
	 * {type: []} = {}
	 */
	res = set.union({type: [] } , { type: ['new','prep'] }, prop);
	assert.deepEqual(res, { }, "intersection");
...
QUnit.test('enum set.equal', function(assert) {
	...
	assert.deepEqual(res, false, "proper subset is not the universal set");
...
QUnit.test('enum set.subset', function(assert) {
	var prop = props["enum"]('type',['new','prep','deliver','delivered']);
	/*
